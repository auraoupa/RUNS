...
#SBATCH --exclusive
 set -x
 cd /scratch/cnt0024/hmg2840/albert7a/TMPDIR_NACHOS12.L75-MAA14
 . ./includefile.ksh
 ...
 for member in $(seq -1 -1) ; do
   ...
   zrstdir='./'
   if [ $RST_DIRS = 1 ] ; then zrstdir=/scratch/cnt0024/hmg2840/albert7a/NACHOS12.L75-MAA14-RST.$ext/$nnn ; fi
...
#!/bin/ksh
   ...
   # O C E A N
   # *********
   OCE_RST_IN=@(LookInNamelist cn_ocerst_in namelist_oce.1)$mmm
   OCE_RST_OUT=@(LookInNamelist cn_ocerst_out namelist_oce.1)$mmm
   ...
   # *****
   if [ 1 = 1 ] ; then
     ICE_RST_IN=@(LookInNamelist cn_icerst_in namelist_ice.1)$mmm
     ICE_RST_OUT=@(LookInNamelist cn_icerst_out namelist_ice.1)$mmm
   ...
   # *****************************
   if [ 0 = 1 ] ; then
     TRC_RST_IN=@(LookInNamelist   cn_trcrst_in namelist_top.1)$mmm
     TRC_RST_OUT=@(LookInNamelist cn_trcrst_out namelist_top.1)$mmm
   ...
   # ************
   if [ 0 = 1 ] ; then
      TRD_RST_IN=@(LookInNamelist cn_trdrst_in namelist_oce.1)$mmm
      TRD_RST_OUT=@(LookInNamelist cn_trdrst_out namelist_oce.1)$mmm
   ...
   # *****
   if [ 0 = 1 ] ; then
      STO_RST_IN=@(LookInNamelist cn_storst_in namelist_oce.1)$mmm
      STO_RST_OUT=@(LookInNamelist cn_storst_out namelist_oce.1)$mmm
...
eof1
   cat ztmp | sed -e 's/@/$/g' > ./zsrst.1.ksh${mmm}.ksh    # change @ into $ and create script for current member
   chmod 755 ./zsrst.1.ksh${mmm}.ksh                         # made it executable

   mpmd_arg="$mpmd_arg 1 ./zsrst.1.ksh${mmm}.ksh"           # prepare the command line for runcode function
 done
  
  pwd
  if [ ! $mmm ] ; then
     ./zsrst.1.ksh${mmm}.ksh                                 # not an ensemble run : serial process
  else
