...
#!/bin/ksh
   ...
   # O C E A N
   # *********
   OCE_RST_IN=@(LookInNamelist cn_ocerst_in namelist_oce.8)$mmm
   OCE_RST_OUT=@(LookInNamelist cn_ocerst_out namelist_oce.8)$mmm
   ...
   # *****
   if [ 1 = 1 ] ; then
     ICE_RST_IN=@(LookInNamelist cn_icerst_in namelist_ice.8)$mmm
     ICE_RST_OUT=@(LookInNamelist cn_icerst_out namelist_ice.8)$mmm
   ...
   # *****************************
   if [ 0 = 1 ] ; then
     TRC_RST_IN=@(LookInNamelist   cn_trcrst_in namelist_top.8)$mmm
     TRC_RST_OUT=@(LookInNamelist cn_trcrst_out namelist_top.8)$mmm
   ...
   # ************
   if [ 0 = 1 ] ; then
      TRD_RST_IN=@(LookInNamelist cn_trdrst_in namelist_oce.8)$mmm
      TRD_RST_OUT=@(LookInNamelist cn_trdrst_out namelist_oce.8)$mmm
   ...
   # *****
   if [ 0 = 1 ] ; then
      STO_RST_IN=@(LookInNamelist cn_storst_in namelist_oce.8)$mmm
      STO_RST_OUT=@(LookInNamelist cn_storst_out namelist_oce.8)$mmm
...
eof1
   cat ztmp | sed -e 's/@/$/g' > ./zsrst.8.ksh${mmm}.ksh    # change @ into $ and create script for current member
   chmod 755 ./zsrst.8.ksh${mmm}.ksh                         # made it executable

   mpmd_arg="$mpmd_arg 1 ./zsrst.8.ksh${mmm}.ksh"           # prepare the command line for runcode function
 done
  
  pwd
  if [ ! $mmm ] ; then
     ./zsrst.8.ksh${mmm}.ksh                                 # not an ensemble run : serial process
  else
