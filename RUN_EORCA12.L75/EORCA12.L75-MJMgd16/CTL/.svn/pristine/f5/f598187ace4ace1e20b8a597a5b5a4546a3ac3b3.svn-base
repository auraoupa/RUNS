...
MAILT=Jean-Marc.Molines@lgge.obs.ujf-grenoble.fr
...
hostname | grep -q login
if [ $? = 0 ] ; then
SUBMIT_SCRIPT=${CONFIG_CASE}_occigen2.ksh   # name of the script to be launched by run_nemo in CTL
else
SUBMIT_SCRIPT=${CONFIG_CASE}_occigen2.ksh   # name of the script to be launched by run_nemo in CTL
fi
...
P_FOR_DIR=${WORKDIR}/DATA_FORCING/DFS5.2_RD/ALL
...
BATFILE_METER=EORCA12_bathymetry_v2.5.nc                                    ; OPA_BATFILE_METER=bathy_meter.nc

# Coordinates
COORDINATES=EORCA12_coordinates.nc                                          ; OPA_COORDINATES=coordinates.nc

# Ice initialiazation/damping
ICEINI=EORCA12.L75-MAL95_y1998-2007m01_icemod_initMAL101.nc                 ; OPA_ICEINI=Ice_initialization.nc
ICEDMP=                                                                     ; OPA_ICEDMP=ice_damping.nc

# Bottom friction enhancement
BFR=EORCA12_bfr2d_UKmod.nc                                                  ; OPA_BFR=bfr_coef.nc  # enhanced bottom coef for Torres

# 3D damping mask (aka AABW stuff)
WDMP=EORCA12.L75_dmp_mask.nc                                                 ; OPA_WDMP=dmp_mask.nc
