...
usage() {
      echo USAGE: $( basename $0 ) [-h ] [-s istep ] [ -b nblock ] [ -f filename ] [ -m ] [ -g ] [-t graphtype] [ -v ] [ -l ]
      ...
      echo "   -f filename : nemo_jade.oxxx file name to consider [ $filename ]"
...
# filter the input file from task information (when running on curie with XIOS)
